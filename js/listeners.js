...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    function addMusicPlayerCloseButtons() {
        if (document.getElementById('close-player-expand')) return;
        const player = document.getElementById('player');
        const minimizeBtn = document.getElementById('minimize-btn');
        const miniView = document.getElementById('mini-view');

        if (minimizeBtn && minimizeBtn.parentNode) {
            const closeExpand = document.createElement('button');
            closeExpand.id = 'close-player-expand';
            closeExpand.innerHTML = '<i class="fas fa-times"></i>';
            closeExpand.className = 'btn close-player-btn';
            closeExpand.style.marginRight = '8px';
            closeExpand.title = '关闭播放器';
            closeExpand.addEventListener('click', (e) => {
                e.stopPropagation();
                closeMusicPlayer();
            });
            minimizeBtn.parentNode.insertBefore(closeExpand, minimizeBtn);
        }

        if (miniView && !miniView.querySelector('.close-mini-player')) {
            const closeMini = document.createElement('button');
            closeMini.className = 'close-mini-player';
            closeMini.innerHTML = '<i class="fas fa-times"></i>';
    ...
    function closeMusicPlayer() {
        ...
        if (typeof showNotification === 'function') {
            showNotification('音乐播放器已关闭', 'info', 1500);
        }
    }

    addMusicPlayerCloseButtons();
