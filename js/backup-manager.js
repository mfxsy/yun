...
(function() {
    ...
    function showToast(text, type, duration) {
        if (typeof window.showToast === 'function') {
            window.showToast(text, type, duration);
        } else {
            alert(text);
        }
    }
    ...
    async function exportFullBackup() {
        try {
            ...
            const fullData = {
                ...
                sessionId: window._SESSION_ID,
                messages: window.messages || [],
                partnerName: window.partnerName || '梦角',
                myName: window.myName || '我',
                isDark: window.isDark || false,
                lastMsgId: window.lastMsgId || 0,
                cards: cardData,
                emojis: emojiData,
                avatars: avatarData
            };
    ...
    async function importFullBackup(file) {
        try {
            ...
            // 直接修改 window 上的全局变量，保证与主程序共享同一数据源
            ...
            document.getElementById('contactName').textContent = window.partnerName;
    ...
    function exportSingleModule(moduleType) {
        ...
        switch (moduleType) {
            ...
            case 'textEmojis':
                ...
                break;
            case 'avatar':
                data = window.avatarManager ? window.avatarManager.exportData() : {};
        ...
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        showToast(`导出 ${moduleType} 成功`, 'success');
    }
            ...
            switch (moduleType) {
                ...
                case 'cards':
                    ...
                    try {
                        ...
                        if (added > 0) {
                            ...
                            showToast(`成功导入 ${added} 条字卡${mode === 'overwrite' ? '（覆盖）' : '（合并）'}`, 'success');
                        } else {
                            showToast('没有新字卡可导入（可能已存在）', 'warning');
                        }
                    } catch (err) {
                ...
                case 'textEmojis':
                    ...
                    try {
                        ...
                        if (added > 0) {
                            await window.cardManager.reload();
                            showToast(`成功导入 ${added} 条 Emoji${mode === 'overwrite' ? '（覆盖）' : '（合并）'}`, 'success');
                        } else {
                            showToast('没有新 Emoji 可导入（可能已存在）', 'warning');
                        }
                    } catch (err) {
                        showToast('导入失败: ' + err.message, 'error');
                    }
                ...
                case 'emojis':
                    ...
                    if (result.success) {
                        ...
                        showToast('表情包导入成功', 'success');
                    } else showToast(result.message, 'error');
                    break;

                case 'avatar':
                    if (!window.avatarManager) return;
                    result = await window.avatarManager.importData(data, mode);
                    if (result.success) {
                        await window.avatarManager.reload();
                        showToast('头像/背景导入成功', 'success');
                    } else showToast(result.message, 'error');
                    break;

                default: showToast('未知模块', 'error');
            }
        } catch (err) {
            showToast('导入失败: ' + err.message, 'error');
            console.error(err);
    ...
    function init() {
        ...
        if (exportFullBtn) exportFullBtn.addEventListener('click', exportFullBackup);
        ...
        if (refreshBtn) refreshBtn.addEventListener('click', updateStorageInfo);
    ...
    window.backupManager = {
        ...
        exportSingleModule: exportSingleModule,
        importSingleModule: importSingleModule,
        updateStorageInfo: updateStorageInfo,
        clearCurrentSession: clearCurrentSession,
        resetAllData: resetAllData
    };

    console.log('✅ backupManager 已加载');
})();